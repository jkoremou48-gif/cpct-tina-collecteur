...
function construireLigneTournante(m, afficherBoutonNouveauContrat) {
  ...
  row.innerHTML = `
    <div>
      ...
      <span class="badge" style="background:#e9ecef; color:#333; margin-left:6px;">${LABEL_TOURNANTE}</span><br>
...
async function afficherDetailsTournante(m) {
  ...
  let operations = [];
  try {
    const mod = await obtenirModuleTournante();
    if (mod) {
      const r = await mod.chargerSoldeSecurite(m.id);
      solde = r.solde;
  ...
  ouvrirModal(`
    <h2>${esc(m.nom)}</h2>
    <p class="subtitle-sm">Téléphone : ${esc(m.telephone || '—')} · ${esc(caisse ? caisse.nom : 'Caisse')} · ${LABEL_TOURNANTE}</p>
...
async function chargerCaissesDansSelect(selectId, infoId) {
  ...
  let moduleIndisponible = false;
  try {
    const mod = await obtenirModuleTournante();
    if (mod) {
      caissesOuvertesCache = await mod.listerCaissesOuvertes(state.currentCollecteurData.uid);
    } else {
      moduleIndisponible = true;
    }
  } catch (err) {
    console.error(err);
    caissesOuvertesCache = [];
  ...
  if (caissesOuvertesCache.length === 0) {
    ...
    if (info) {
      info.textContent = moduleIndisponible
        ? "Module tontine tournante indisponible : vérifiez que le fichier js/tournante-commun.js est bien publié."
        : "Aucune caisse tournante n'est ouverte aux inscriptions. Le PDG doit en créer une pour votre compte.";
    }
    return;
  }
...
function renderMembersList() {
  ...
  contratsAffiches.forEach((contrat) => {
    ...
    row.innerHTML = `
      <div>
        ...
        ${propositionEnAttente ? `<br><small style="color:#e67e22;">Nouveau contrat proposé (${propositionEnAttente.type_contrat === 'tournante' ? LABEL_TOURNANTE : infoTypeContrat(propositionEnAttente.type_contrat || 'journalier').label}) — en attente de confirmation du membre</small>` : ''}
  ...
  // (isolées : une erreur ici ne bloque pas le reste de la liste) ---
  try {
    ...
    console.warn('Lignes tontine tournante :', err);
  }
}
    ...
    <form id="form-nouveau-contrat">
      <div class="field-row">
        ...
        <select name="typeContrat" id="select-type-contrat-nc" required>
          ...
          <option value="tournante">${LABEL_TOURNANTE}</option>
        </select>
      </div>
